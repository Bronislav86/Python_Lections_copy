...
# на на туже область в памяти, что и функция и потом использовать функцию, вызывая её через имя переменной
...
# Второй вариант решеине - через кортеж
...
# def select(f, col):
#     return [f(x) for x in col] # Эта ф-я возвращает список, в котором мы к каждому элприменили функцию f
